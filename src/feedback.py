    ...
    for word, pairs in word_phone_pairings.items():
        for target_phoneme, speech_phoneme in pairs:
            if target_phoneme != speech_phoneme:
                ...
                mistake_count, word_set, mistake_severities, phoneme_spoken_as = (
                    phoneme_mistake_freq[target_phoneme]
                )
                word_set.add(word)
                mistake_severities.append(
                    dist.feature_error_rate(target_phoneme, speech_phoneme)
                )
...
def feedback(target, target_by_words, speech, good_enough_threshold=0.4):
    ...
    for word, pairs in pbw:
        ...
        for p in pairs:
            dist = dist.weighted_feature_edit_distance(p[0], p[1])
